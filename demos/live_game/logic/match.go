...
var (
	playerSkinList  = []int{301, 401, 302, 402, 303, 403, 304, 404, 305, 405}
	bubbleColorList = []int{1, 2, 3, 4}
)
...
func (m *Match) JoinRobot(robot *Robot) (err error) {
	logger.Infof("JoinRobot: %v, %v", robot.id, robot.name)
...
func (m *Match) BroadcastMsg(msg *game.Message) {
	for _, p := range m.players {
		p.SendMsg(msg)
	}
}

func randomPlayerSkin() int {
	i := utils.RandInt(0, len(playerSkinList))
	return playerSkinList[i]
}

func randomBubbleColor() int {
	i := utils.RandInt(0, len(bubbleColorList))
	return bubbleColorList[i]
}
